...
typedef struct {
    ...
    int             videoStream;
    uint8_t         *bufferRGBA;
    const char      *codecName;      // Name of the codec being used
    const char      *codecLongName;  // Long name/description of codec
    int             isHardwareAccel; // 1 if hardware accelerated, 0 if software
} Decoder;
...
int init_decoder(const char *filename, Decoder *d) {
    ...
    for (unsigned int i = 0; i < d->formatCtx->nb_streams; i++) {
        if (d->formatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            for (int j = 0; j < decoder_count; j++) {
                ...
                d->codecLongName = codec->long_name ? codec->long_name : "Unknown";

                // Detect if this is a hardware decoder
                // Hardware decoders typically have suffixes: _rkmpp, _vaapi, _nvdec, _v4l2m2m, _videotoolbox
                d->isHardwareAccel = (strstr(codec->name, "_rkmpp") != NULL ||
                                     strstr(codec->name, "_vaapi") != NULL ||
                                     strstr(codec->name, "_nvdec") != NULL ||
                                     strstr(codec->name, "_cuvid") != NULL ||
                                     strstr(codec->name, "_v4l2m2m") != NULL ||
                                     strstr(codec->name, "_videotoolbox") != NULL ||
                                     strstr(codec->name, "_qsv") != NULL);

                fprintf(stderr, "Successfully opened decoder: %s (id=%d) for codec %d [HW=%s]\n",
                        codec->name, codec->id, stream_codec_id,
                        d->isHardwareAccel ? "YES" : "NO");
                break;
            }
                ...
                if (default_codec) {
                    ...
                    if (test_ctx) {
                        ...
                        if (avcodec_open2(test_ctx, default_codec, NULL) >= 0) {
                            ...
                            // Store codec information for fallback decoder
                            d->codecName = default_codec->name;
                            d->codecLongName = default_codec->long_name ? default_codec->long_name : "Unknown";
                            d->isHardwareAccel = 0; // Default fallback is always software

                            fprintf(stderr, "Successfully opened default decoder: %s [HW=NO]\n", default_codec->name);
                        } else {
    ...
    // Allocate RGBA conversion resources
...
// rgba_data points to RGBA buffer
...
// ----------------------------------------------------------------
const char* getCodecName(Decoder *d) {
    return d->codecName ? d->codecName : "unknown";
}

const char* getCodecLongName(Decoder *d) {
    return d->codecLongName ? d->codecLongName : "Unknown codec";
}

int isHardwareAccelerated(Decoder *d) {
    return d->isHardwareAccel;
}

int getCodecID(Decoder *d) {
    if (!d || !d->codecCtx) {
        return 0;
    }
    return d->codecCtx->codec_id;
}
*/
import "C"
...
type videoDecoder struct {
	...
	height            int
	fps               float64
	codecName         string // Name of the codec (e.g., "h264", "hevc")
	codecLongName     string // Full description of codec
	isHardwareAccel   bool   // true if hardware accelerated
	codecID           int    // FFmpeg codec ID
}
...
func newVideoDecoder(path string) (*videoDecoder, error) {
	...
	dec.codecName = C.GoString(C.getCodecName(&dec.cdec))
	dec.codecLongName = C.GoString(C.getCodecLongName(&dec.cdec))
	dec.isHardwareAccel = int(C.isHardwareAccelerated(&dec.cdec)) != 0
	dec.codecID = int(C.getCodecID(&dec.cdec))
	...
	// Log comprehensive decoder information
	hwStatus := "SOFTWARE"
	if dec.isHardwareAccel {
		hwStatus = "HARDWARE"
	}

	log.Printf("Decoder: %s [%s] %dx%d @ %.1ffps | Accel=%s | Output=RGBA",
		dec.codecName, dec.codecLongName, dec.width, dec.height, dec.fps, hwStatus)

	return dec, nil
}

// frameData holds RGBA pixel data
type frameData struct {
	rgba []byte
}

func (d *videoDecoder) nextFrame() (*frameData, error) {
	var data *C.uint8_t
	...
	// Data points to RGBA buffer
	bufLen := d.width * d.height * 4 // RGBA
	return &frameData{
		rgba: C.GoBytes(unsafe.Pointer(data), C.int(bufLen)),
	}, nil
}
...
type Player struct {
	...
	bounce         bool
	bounceFrames   []*frameData // Store frame data for bounce playback
	playingCached  bool
	cacheIdx       int
...
func (p *Player) SetRenderer(renderer *sdl.Renderer) error {
	...
	// Create RGBA texture for video frames
	var err error
...
func (p *Player) updateTexture(frame *frameData) error {
	if p.texture == nil {
		return fmt.Errorf("texture not initialized")
	}

	// Lock and copy RGBA pixel data
...
func (p *Player) UpdateFrame() error {
	...
	// Upload to texture
	if err := p.updateTexture(frameData); err != nil {
		return err
	}
	...
	// Save for bounce
	if p.bounce {
		p.bounceFrames = append(p.bounceFrames, frameData)
	}
...
// CodecInfo contains information about the video codec being used
type CodecInfo struct {
	Name            string  // Short name (e.g., "h264", "hevc", "mpeg1video")
...
func (p *Player) GetCodecInfo() CodecInfo {
	p.m.Lock()
	defer p.m.Unlock()

	return CodecInfo{
		Name:            p.dec.codecName,
		LongName:        p.dec.codecLongName,
		IsHardwareAccel: p.dec.isHardwareAccel,
		CodecID:         p.dec.codecID,
		Width:           p.dec.width,
		Height:          p.dec.height,
		FPS:             p.dec.fps,
	}
...
// IsHardwareAccelerated returns true if the decoder is using hardware acceleration
func (p *Player) IsHardwareAccelerated() bool {
	p.m.Lock()
	defer p.m.Unlock()
	return p.dec.isHardwareAccel
}

// GetCodecName returns the short name of the codec being used
func (p *Player) GetCodecName() string {
	p.m.Lock()
	defer p.m.Unlock()
	return p.dec.codecName
}
